...
from models.tc_clip.model_utils import LayerNorm
from models.tc_clip.transformer import Transformer

# Simplified ToMe - remove token merging for now
def parse_r(num_layers, r_tuple):
    """Parse ToMe reduction schedule."""
    return [0] * num_layers  # No token reduction
...
class TCVisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_layer_num=None, return_attention=False, return_source=False):
        ...
        x, attns, source = self.transformer.forward_tc(x,
                                                       ...
                                                       return_source=return_source)  # [B, n_layer, T+k, D]
        
        # Transpose to [n_layer, B, T+k, D] as expected
        x = x.transpose(0, 1)  # [n_layer, B, T+k, D]
