...
from models.tc_clip.model_utils import LayerNorm, QuickGELU

# Simplified ToMe stubs - we don't use token merging
def bipartite_soft_matching(metric, r, class_token=False, distill_token=False):
    """Stub: returns None for no merging (merge=None means no token reduction)."""
    return None, None  # (merge, unmerge) both None means no reduction

def merge_source(merge, x, source=None):
    """Stub: no merging, just return original."""
    return x

def merge_wavg(merge, x, size=None):
    """Stub: no merging, return original tensor and size."""
    return x, size

def schedule_r_constant(start_number, final_number, r):
    """Stub: return zero reduction schedule."""
    return [0], [0]
