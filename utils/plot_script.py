...
def plot_3d_motion_camera(save_path, joints, title, figsize=(10, 10), fps=30, radius=4):
    """
    Plot 3D camera trajectory (single joint) as a path in 3D space.
    """
    matplotlib.use('Agg')
    ...
    height_offset = MINS[1]
    data[:, :, 1] -= height_offset
    trajec = data[:, 0, [0, 2]]  # Camera trajectory (x, z)
    ...
    def update(index):
        ...
        # Plot camera trajectory path
        if index > 1:
            ax.plot3D(trajec[:index, 0] - trajec[index, 0], np.zeros_like(trajec[:index, 0]),
                      trajec[:index, 1] - trajec[index, 1], linewidth=2.0,
                      color='blue', alpha=0.7)

        # Plot current camera position as a sphere
        ax.scatter(data[index, 0, 0], data[index, 0, 1], data[index, 0, 2], 
